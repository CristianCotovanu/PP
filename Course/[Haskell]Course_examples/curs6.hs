...
-- --ex: de implementat filter cu case
-- filterCase p [] = []
-- filterCase p (x:xs) = case p x of
-- 	False -> filterCase p xs
-- 	_ -> x : filterCase p xs
...
-- --ex: de implementat filter cu gărzi
-- filterGuards p [] = []
-- filterGuards p (x:xs)
-- 	| p x = x : filterGuards p xs
-- 	| otherwise = filterGuards p xs
...
-- 	where
-- 	a = 2+5
-- 	(b, c) = head (zip [1..] [2..]) -- (b,c) = (1,2), b=1, c=2
-- 	d = f 5
-- 		where f = (+1)
		
-- --ex: de implementat filter cu gărzi și where
-- filterWhere p [] = []
-- filterWhere p (x:xs)
-- 	| p x = x : recCall
-- 	| otherwise = recCall
-- 		where recCall = filterWhere p xs
...
-- i x y = if y>5 then y*y else x*x
...
-- f "abcd"
-- f :: -> Bool
-- f = if [1..4] == [1..5] then True else False

-- f

foldl (\ y x -> (:) (head x) y) [] [[x] | x <- [1..10], x 'mod' 2 == 0]