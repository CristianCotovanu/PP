...
au_pusca(ListaNume) :- findall(Om, inarmat(Om, pusca), ListaNume).
...
au_arma(Arma, ListaNume) :- findall(Om, inarmat(Om, Arma), ListaNume).

au_marca(Marca, ListaNume) :- findall(Om, conduce(Om, Marca), ListaNume).
...
arme_bmw(ListaArme) :- findall(Arma, (conduce(Om, bmw), inarmat(Om, Arma)), ListaArme).
...
arme_marca(Marca, ListaArme) :- findall(Arma, (conduce(Om, Marca), inarmat(Om, Arma)), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, (conduce(_, Marca), arme_marca(Marca, ListaArme), single_elem(ListaArme)), ListaMarci).
...
suspect1(Nume:Marca:Arma) :- suspect(Nume:Marca:Arma), findall(Arma, inarmat(_, Arma), ArmeUnice), not(single_elem(ArmeUnice)).

multiples(D,L):-
    digits(Digits),
    numbers(Numbers),
    findall(N,(member(D, Digits), member(N, Numbers), N mod D =:= 0), L).
...
suspect2(Nume:Marca:Arma) :- suspect1(Nume:Marca:Arma), findall(Marca, conduce(_, Marca), MarciUnice), not(single_elem(MarciUnice)).
...
suspect3(Nume:Marca:Arma) :- suspect1(Nume:Marca:Arma). 
...
suspect4(Nume:Marca:Arma) :- suspect1(Nume:Marca:Arma), suspect2(Nume:Marca:Arma), suspect3(Nume:Marca:Arma). 
