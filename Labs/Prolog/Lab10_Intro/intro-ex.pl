...
myConcat([], L, L):- true.
myConcat(L, [], L):- true.
myConcat([Head | Rest], L, [Head | Rest2]):- myConcat(Rest, L, Rest2).
...
myReverse([], []):- true.
myReverse([Head | Rest], List) :- myReverse(Rest, X), myConcat(X, [Head], List).
...
myReverseAcc([], Acc, Acc) :- true.
myReverseAcc([Head | Rest], Acc, List) :- myReverseAcc(Rest, [Head | Acc], List).
...
extract([],_,_,[]):- true.
extract([Head | Rest], 0, EndOld, [Head | List]) :- EndNew is EndOld - 1, extract(Rest, 0, EndNew, List).
extract([_ | Rest], StartOld, EndOld, List) :- StartNew is StartOld - 1, EndNew is EndOld - 1, extract(Rest, StartNew, EndNew, List).
...
factorial(N, 1):- N < 2.
factorial(N, Fact) :- NewN is N - 1, factorial(NewN, NewFact), Fact is N * NewFact.
...
factorial2(0, 1) :- true.
factorial2(N, Fact) :- factorial2(NewN, NewFact), N is NewN + 1, Fact is N * NewFact.
...
sublista([], []) :- true.
sublista([Head | Rest], [Head | L]) :- sublista(Rest, L).
...
isLeaf() :- false.
isLeaf(Nod) :- nod(Nod), not(arc(Nod, _)).
...
isRoot(Nod) :- nod(Nod), not(arc(_, Nod)).
