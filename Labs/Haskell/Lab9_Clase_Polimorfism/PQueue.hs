...
class (Ord a) => PQueue pq a where
    ...
    toList prioQueue
        | isEmpty prioQueue = []
        | otherwise = (fromJust $ top prioQueue) : (toList $ pop prioQueue)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert x (LPQ prioQueue)
      | null prioQueue = LPQ [x]
      | fst(head prioQueue) > fst x = LPQ (head prioQueue : toList (insert x (LPQ (tail prioQueue))))
      | otherwise = LPQ (x : prioQueue)

    top (LPQ prioQueue)
      | null prioQueue = Nothing
      | otherwise = Just $ head prioQueue

    pop (LPQ prioQueue) = LPQ (tail prioQueue)

    isEmpty (LPQ prioQueue) = null prioQueue
...
merge t1@(Node rang1 (p1, value1) left1 right1) t2@(Node rang2 (p2, value2) left2 right2)
  | p1 <= p2 = merge t2 t1
  | rank left1 < rank (merge right1 t2) = (Node ((rank left1) + 1) (p1, value1) (merge right1 t2) left1)


inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node rang x left right) = inorder left ++ [x] ++ inorder right
