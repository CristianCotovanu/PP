...
primes 0 = []
primes 1 = []
primes n = [x | x <- [2..n], and [(mod x y) /= 0 | y <- [2..(div x 2)]]]
...
setIntersection [] _ = []
setIntersection _ [] = []
setIntersection a b = if (elem (head a) b) then head a : setIntersection (tail a) b else setIntersection (tail a) b
-- setIntersection a b = [x | x <- b, (elem x a)]
...
setUnion :: Eq a => [a] -> [a] -> [a]
-- setUnion a b = [x | x <- a, y <-b, (not (elem x (setUnion a b)))]
setUnion a b = setDiff b a ++ a
...
-- cntConsec :: [a] -> a
-- cntConsec [] = 0
-- cntConsec (a : rest) = if a == (head rest) then 1 + (cntConsec rest) else 0

group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 [x] = [[x]]
group2 (x : rest)
      | (x == head rest) = (x : head (group2 rest)) : tail (group2 rest)
      | otherwise = [x] : group2 rest
...
-- nrOcc = undefined
nrOcc [] = []
nrOcc x = map (\x -> ((head x), (length x))) (group (sort x))
...
dup = undefined
-- dup "" = ""
-- dup strings = [(x ++ " " ++ x) | x <- (unwords strings)]
