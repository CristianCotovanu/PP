...
myCycle :: [a] -> [a]
myCycle [] = []
myCycle x = newX where newX = x ++ newX
...
arithmetic init ratio = iterate (\x -> x + ratio) init
geometric :: Num a => a -> a -> [a]
geometric init ratio = iterate (\x -> x * ratio) init
{-
  3. (2p)
...
wordToAlpha word = alpha
    where alpha = chr (mod (ord $ wordToChar word) nrLetters + a)
          a = 97
          nrLetters = 26
{-
    5. (1p)
...
randomAlphaKey seed = map wordToAlpha (randoms seed)
...
tableToFunc table searchFor = let uncryptedChar = snd $ head table
                                  key = fst $ head table 
                              in if (searchFor == key) then uncryptedChar
                              else tableToFunc (tail table) searchFor
        
substCrypt :: [(Char, Char)] -> String -> String
substCrypt table str = map (\c -> (tableToFunc table c)) str
...
genRotTable offset = zip alphabet rot13Alphabet
      where
        alphabet = ['a'..'z']
        rot13Alphabet = map (\letter -> chr ((mod (offset + ord letter - a) nrLetters) + a)) alphabet
        a = 97
        nrLetters = 26
...
encryptVigenere textToEncrypt key = zipWith tableToFunc rotTables textToEncrypt
        where
