...
(struct bird_state (x y vy) #:transparent)

(struct pipe_state (gap_x gap_y) #:transparent)

(struct state (bird_state pipes score abilities variables) #:transparent)
...
(define (get-initial-state)
  (state (bird_state bird-x bird-initial-y 0)
   (list (pipe_state scene-width (+ added-number (random random-threshold)))) 0 null null))
...
(define (next-state-bird bird gravity)
    (struct-copy bird_state bird
                 [y (+ (get-bird-y bird) (get-bird-v-y bird))]
                 [vy (+ (get-bird-v-y bird) gravity)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy state current-state [bird_state (next-state-bird-onspace (get-bird current-state) initial-momentum)])
      current-state))
...
(define (get-pipe-x pipe)
  (pipe_state-gap_x pipe))

(define (get-pipe-y pipe)
  (pipe_state-gap_y pipe))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (pipe) (struct-copy pipe_state pipe [gap_x (- (get-pipe-x pipe) scroll-speed)])) pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (append pipes (list (pipe_state (+ pipe-width pipe-gap (get-pipe-x (car pipes))) (+ added-number (random random-threshold)))))
      pipes))
...
(define (check-pipe-collisions bird pipes)
  (or
   (check-collision-rectangles
    (make-posn (get-bird-x bird) (get-bird-y bird)) (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
    (make-posn (get-pipe-x (car pipes)) 0) (make-posn (+ (get-pipe-x (car pipes)) pipe-width) (get-pipe-y (car pipes))))

   (check-collision-rectangles
    (make-posn (get-bird-x bird) (get-bird-y bird)) (make-posn (+ (get-bird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
    (make-posn (get-pipe-x (car pipes)) (+ (get-pipe-y (car pipes)) pipe-self-gap)) (make-posn (+ (get-pipe-x (car pipes)) pipe-width) ground-y))))
...
(define (next-state my_state)
  (struct-copy state my_state
               [bird_state (next-state-bird (get-bird my_state) initial-gravity)]
               [pipes (next-state-pipes (get-pipes my_state) initial-scroll-speed)]
               [score (+ (get-score my_state) 0.1)]))
...
(define pipe-gap-image (rectangle pipe-gap pipe-self-gap "solid" "white"))
...
(define (place-one-pipe pipe scene)
  (place-image pipe-gap-image
               (+ (get-pipe-x pipe) (quotient pipe-width 2))
               (+ (get-pipe-y pipe) (quotient pipe-self-gap 2))
               (place-image pipe-image
                            (+ (get-pipe-x pipe) (quotient pipe-width 2))
                            (quotient scene-height 2)
                            scene)))

(define (place-pipes pipes scene)
	(foldl place-one-pipe scene pipes))
...
;(struct variables (gravity momentum scroll_speed) #:transparent)
...
(define (get-variables-scroll-speed variables)
  'your-code-here)
  ;(variables-scroll_speed variables))
